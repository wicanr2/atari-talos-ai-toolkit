...
招募後衣袍取放也已通過：正常右鍵開物品欄、取下 ROBE、眼睛檢視 0.4 KG、
放入第一個背包槽再穿回。新增四個畫面雜湊，冷啟動重播一致；完整 Go 測試
（含外部 CPU 語料）與 vet 通過。這是單一物品路徑，不代表所有物品或遊戲寫盤。
remake 規格 72 已修正持物眼睛及放開恢復；正常路徑 ROBE／0.4 KG 與
食物水面板核對本原片收據一致。此輪僅更新 Talos 驗證文件，核心不變。
第一層（地圖 0）至第二層（地圖 1）也已從冷啟動正常走通；ELIJA 招募後
經踏板開門與樓梯，下樓後前進、左轉看到木門。四個新增畫面檢查點連同既有
三點重播一致，輸入在 `docs/dm12en-stairs-actions.json`；未修改核心或遊戲記憶體。
下一個驗證閘門與重跑方式見
