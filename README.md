...
227,500 筆外部語料，且已保留語料中的 idle／active bus 時間軸；machine epoch、
timed Bus 與首個 4-clock prefetch 路徑已接線。ST／STF memory map
與固定 EmuTOS ROM 的前 18 條／496 clocks 已逐狀態對上 Hatari。畫面、輸入、磁碟與完整 TOS
...
| M2 | ST／STF 記憶體、TOS 開機與決定性時鐘 | **基礎 memory map、power-on reset、MMU、exception、`RESET`、空 cartridge 與首個 external bus slot 切片已 CONFORMED；固定 EmuTOS 前 18 條／496 clocks 對上 Hatari，下一停點為 line-F／vector 11** |
