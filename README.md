...
7,519 條指令；下一個明確停止點是 `$FFFA19` MFP Timer A Control Register byte write。畫面、輸入、磁碟與完整 TOS
...
| M2 | ST／STF 記憶體、TOS 開機與決定性時鐘 | **基礎 memory map、power-on reset、MMU、exception、`RESET`、空 cartridge、line-F、ST void I/O、`TST.B` bus error、GPIP／AER／DDR／IER／IPR／ISR／IMR／VR 與首個 external bus slot 切片已 CONFORMED；固定 EmuTOS 可走 7,519 條，下一停點為 `$FFFA19` MFP Timer A Control Register byte write** |
