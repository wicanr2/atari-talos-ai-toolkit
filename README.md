...
227,500 筆外部語料，ST／STF memory map 與固定 EmuTOS ROM 的前 11 條指令已逐狀態
...
| M2 | ST／STF 記憶體、TOS 開機與決定性時鐘 | **基礎 memory map、power-on reset、MMU、`MOVEC`→vector 4、absolute-short vector 2 frame 與 `RESET`（11 條／352 clocks）已對 Hatari CONFORMED；下一差異是 `$FA0000` cartridge window，其餘 I/O 與後續開機進行中** |
