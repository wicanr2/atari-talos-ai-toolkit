...
7,598 條指令；下一個明確停止點是 PC `$FCD09E` 的 MC68000 `STOP`（opcode `$4E72`）。畫面、輸入、磁碟與完整 TOS
...
| M2 | ST／STF 記憶體、TOS 開機與決定性時鐘 | **基礎 memory map、power-on reset、MMU、exception、`RESET`、空 cartridge、line-F、ST void I/O、`TST.B` bus error、MFP control／timer-data／USART reset 切片與首個 external bus slot 已 CONFORMED；固定 EmuTOS 可走 7,598 條，下一停點為 PC `$FCD09E` 的 MC68000 `STOP`** |
