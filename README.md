...
與固定 EmuTOS ROM 的前 19 條／532 clocks 已逐狀態對上 Hatari，line-F／vector 11
已完成。畫面、輸入、磁碟與完整 TOS
...
| M2 | ST／STF 記憶體、TOS 開機與決定性時鐘 | **基礎 memory map、power-on reset、MMU、exception、`RESET`、空 cartridge、line-F 與首個 external bus slot 切片已 CONFORMED；固定 EmuTOS 前 19 條／532 clocks 對上 Hatari，後續可走 6,850 條，下一停點為 `$FF860F` 保留 I/O** |
