...
7,599 條指令並正確進入 MC68000 stopped state；下一個開機閘門是 interrupt／trace／
external reset 喚醒與對應周邊 IRQ。畫面、輸入、磁碟與完整 TOS
...
| M2 | ST／STF 記憶體、TOS 開機與決定性時鐘 | **基礎 memory map、power-on reset、MMU、exception、`RESET`／`STOP`、空 cartridge、line-F、ST void I/O、`TST.B` bus error、MFP control／timer-data／USART reset 切片與首個 external bus slot 已 CONFORMED；固定 EmuTOS 可走 7,599 條並停機，下一閘門為 interrupt 喚醒與周邊 IRQ** |
