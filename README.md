...
| M2 | ST／STF 記憶體、TOS 開機與決定性時鐘 | **基礎 memory map、power-on reset、MMU `$FF8001` 與 EmuTOS `MOVEC`→vector 4（8 條／128 clocks）與 `$FC0080` 的 vector 2 bus-error frame（`$FFFF8006`）已對 Hatari CONFORMED；下一項是 `$FC0088` 的 `RESET`，其餘 I/O 與後續開機進行中** |
