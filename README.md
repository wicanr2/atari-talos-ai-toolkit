...
| M2 | ST／STF 記憶體、TOS 開機與決定性時鐘 | **基礎 memory map、power-on reset、MMU、exception、`RESET`、空 cartridge、line-F、ST void I/O 與首個 external bus slot 切片已 CONFORMED；固定 EmuTOS 可走 6,916 條，下一停點為 `$FF8A3C` Blitter 探測的 `TST.B` bus error** |
