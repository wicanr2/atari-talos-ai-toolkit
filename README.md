...
讀回`$E4`並清除IRQ也已與固定Hatari trace一致。
輸入、磁碟與完整 TOS
...
| M3 | Shifter 畫面、鍵鼠、FDC 與磁碟映像 | **低解析度索引畫面及FDC force-interrupt／restore／status read-clear首切片已 CONFORMED；RGB／PNG、鍵鼠、seek與磁碟映像尚未完成** |
