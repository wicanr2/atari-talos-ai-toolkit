...
- 分派迴圈在偏移 `$00DE`，緊接在分派表 `$00EC` 之前：`moveq #0,d0` /
  `move.b (a4)+,d0` / `add.w d0,d0` / `move.w 6(pc,d0.w),d7` / `jmp 0(a3,d7.w)`。
  `6(pc,d0.w)` 的延伸字在 `$00E6`，加 6 正好落在分派表——這是「表在 `$00EC`」的第二個
  獨立證據（另一個是直接讀表內容）。`jmp 0(a3,d7.w)` 說明表項是相對載入基底的偏移，
  不是絕對位址，所以直譯器不必載在位址 0。
- 執行模型：`a3` 載入基底、`a4` p-code IP、`a5` 迴圈位址、`a7` 運算元堆疊、
  `a0` 活動記錄。短常數族每條 p-code 耗 8 條 68000 指令（迴圈 5 ＋ 常式 3）。
- 更正一則外部斷言：SunDog 專案文件原記「`SYSTEM.INTERP` 零個 trap」，實際上偏移
  `$008A` 有一條由旗標保護的 `TRAP #15` 除錯 hook。另有兩處看似 trap 的位元組在
  `$14CE`、`$151C`，是字串 `"FUNCTION"` 與 `"NOT "` 的內容。全檔沒有 `TRAP #1`／`#13`／
  `#14`，所以「不呼叫 GEMDOS／BIOS／XBIOS」成立。位元組序列不等於程式碼。
- 分派表全解：256 個 opcode → 107 支常式。`$0304` 收了 45 個（`$40`–`$5F`、`$AA`、`$AF`、
  `$F5`–`$FF`），首指令 `moveq #11,d0`，是無效 opcode 的錯誤路徑；`$9C` 的表項就是迴圈
  起點 `$00DE`，等於 NOP。
- `$0554`（取區域變數位址）是 `subi.w #$BE,d0` / `add.l a0,d0` / `sub.l a6,d0` /
  `addq.w #8,d0`：算出主機位址後減掉 `a6`。`a6` 是 p-system 記憶體基底，所以推上堆疊的
  是 p-system 位址。`$057C`（存入區域變數）是 `move.w (sp)+,8(a0,d0.l)`，與 `$0534` 對稱。
  三者共用同一條 `frame+8+n×2`。
- p-code 對拍：同一段 p-code 分別由 Atari Talos 執行原版 `SYSTEM.INTERP`（68000）與
  laanwj/sundog 的 `psys_interpreter.c`（獨立 C 重寫）執行，六組逐字相同——四條短常數
  `[0,31,3,5]`；`ldl3`／`sldc7`／`ixa $14` 得 `$1118`；`sldc21`／`sstl1`／`ldl1` 往返得
  `[21]` 且區域變數 1 為 21；`slla1` 得 `$800A`；兩條 NOP 加短常數得 `[5]`、IP +3；
  `ixa` 兩位元組運算元得 `$1400`。對拍排除「其中一邊理解錯了」，不排除「兩邊照同一份
  錯誤文件寫」；但一邊是 1985 年的原版機器碼，該風險只落在 C 重寫那一側。
- 算術族的偏移與語意：`$80 ldcb`→`$050C`、`$81 ldci`→`$0514`（運算元 little-endian）、
  `$A2 adi`→`$0A44`、`$A3 sbi`→`$0A4A`（`tos1 − tos0`）、`$8D dvi`→`$0A58`、
  `$8F modi`→`$0A6A`、`$B0 equi`→`$0A98`、`$B2 leqi`→`$0AB0`、`$E2 dup1`→`$0994`、
  `$BD swap`→`$098C`。比較運算把布林推成 word 1 或 0。
- SunDog `check_exit` 的格座標換算寫成 p-code 跑過：`(7 + 175/40) mod 40 = 11`、
  `(4 + 120/40) mod 24 = 7`、再 `ixa 20` 得 `$1118`。捲動、sprite 座標與步距都是原版
  執行時用除錯器讀出來的，算出的欄列與同一刻讀到的格座標一致。

- 布林族的偏移與語意：`$A1 land`→`$0A02`（`and.w d0,(a7)`）、`$A0 lor`→`$0A08`
  （`or.w d0,(a7)`）、`$9F bnot`→`$0A12`（`not.w (a7)` 加 `andi.w #1,(a7)`）。
  三支都是**位元**運算，不是邏輯運算：`2 and 1` 得 0、`2 or 4` 得 6。
  配上 `fjp`（`$0D22`）與 `tjp`（`$0D18`）的 `btst #0,d0`，p-system 的真假整套住在
  **bit 0**——8 是假、9 是真。
- 這個語意解掉了 SunDog remake 專案掛了一輪的矛盾。`XSTARTUP:0x31` 的新遊戲初始損壞用
  `(欄 = 0) or random()` 決定壞法，而 `random()` 回 0–8191。當成邏輯或加「非零即真」讀，
  條件幾乎永遠成立，列狀態 3 幾乎不可能出現；原版實測卻是 72 個列狀態裡有 8 個 3。
  照位元或加 bit 0 讀，條件為假的機率是 `3/4 × 1/2 = 3/8`，與實測的 8/23 ＝ 34.8% 相符。
  `TestUCSDInterpNewGameDamageBranch` 把那段判斷式在 Atari Talos 上跑出整張真值表。
