...
| ST DMA mode／WD1772 force-interrupt初始化 | **CONFORMED** | mode `$0080`選command/status、`$D0`清IRQ並建立Type-I status；289,612條／2,983,694 clocks |
...
| ST／STF I/O memory map | 進行中 | recurring VBL、video、MFP、PSG／ACIA／USART與首組DMA/FDC init已接；下一步第二組mode `$0080`、restore `$0B`與IRQ期限 |
