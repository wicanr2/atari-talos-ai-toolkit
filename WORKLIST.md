...
| ST MIDI ACIA control／IKBD stale RDR／MFP channel 6 | **CONFORMED** | `$03→$95`、一次`$F1`保留值讀取、IERB/IMRB=`$60`；136,182條／1,578,882 clocks抵達Timer D重設 |
| ST MFP Timer D system-clock start | **CONFORMED** | channel 4 clear、TDDR=`$00`=256、IERB/IMRB=`$70`、TCDCR=`$52`；136,210條／1,579,228 clocks |
| ST MFP Timer D recurrence／channel 4 IRQ | **CONFORMED** | 2,560 MFP clocks 有理數週期、IPRB bit 4、level 6 vector 68、software EOI；137,138條／1,587,632 clocks 進 `$FC7884` |
...
| ST／STF I/O memory map | 進行中 | recurring VBL、video mode、palette、programmed／active base、low-res planar、MFP boot init、Timer D IRQ、PSG與兩組ACIA init已接；下一步是Timer C timeout／IRQ |
