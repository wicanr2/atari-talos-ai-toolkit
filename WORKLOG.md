...
- 先確認兩分支 push 最新，再新增 `dm12en-stairs-actions.json`；接續 ELIJA
  招募共 63 動作，踏板開門、下樓前、下樓後及第二層木門四點雜湊冷啟動重播一致。
  clocks=18149864946、Timer B events=402399、IACK=3952。此輪不改核心。
  remake 正常路徑存檔 map 1、(3,0) 面西、ELIJA 60/60；讀檔畫面另有三個
  骷髏欄位與變暗待查，不算完整存讀檔對拍。原版圖像僅留 DM 私人驗證目錄。
  本輪專用 Docker 容器均已退出移除，成果提交並推送原有工作分支，不改 main。

- remake 依原片收據修正持物眼睛與放開恢復，規格 72 正常 GUI 路徑通過；
  衣袍名稱／0.4 KG 與恢復食物水頁均一致。Talos 本輪不改核心、不重跑原片，
  沿用七個雜湊重播證據。依使用者要求推送既有驗證分支，不動 main。

- 招募後 ROBE 路徑：新增可接續既有輸入的 `TALOS_BOOT_ACTIONS_TAIL`，
  與主輸入同樣拒絕未知欄位，合計仍限制 200 動作。不改硬體或遊戲記憶體。
  衣袍取下、眼睛顯示 ROBE／0.4 KG、背包收納及穿回可重現，新增四點雜湊
  冷啟動重跑通過；完整 Go 測試（含外部 CPU 語料）及 vet 通過。
  眼睛按住時間從 1M 延至 5M clocks；原版 F352 先 Delay(8)，短按截圖太早，
  不以改遊戲程式繞過等待。私人收據位於 DM 的 `postrecruit-items/`。
  remake 取放及負重抽樣一致，但持物眼睛錯顯技能、放開後不恢復，另記為
  對拍差異而非 Talos 通過即可外推。專用 Docker 容器已退出移除，不改 main、不推送。

- Timer B 修正：規格 155、正常 50 Hz 顯示事件、channel 8 中斷、延後重載及
  STOP 到期喚醒。Hatari 黑箱 400 事件、內部測試、完整外部 CPU 語料、vet、建置通過。
- 正常原片回放完成 33 個移動／轉向動作及滑鼠開啟 ELIJA、RESURRECT；
  最終顯示 ELIJA RESURRECTED 與隊伍列，Timer B IACK=2305。
  三個檢查點留 SHA-256，沒有注入 PC、角色或座標。這不代表完整遊戲對拍。
  加入雜湊斷言後再從冷啟動重跑，三點全部一致；收據保存私人 `timerb-elija/`。
  本輪專用 Docker 容器全部退出移除，main 未改、未推送；remake 只更新驗證文件。

- 繼續原片驗證：增加診斷測試的 ENTER 後有界等待與 PC／磁頭進度，
  未改硬體實作。15M／50M 步仍在載入；100M 上限於第 51897669 步
  明確碰到 Timer B `$FFFA1B` 寫入拒絕。已畫出地城方向按鈕，不等於可操作。
  同狀態、招募、戰鬥與存讀檔 gate 維持未完成，沒有提高 remake 分數。
  第二次乾淨重跑在相同步數、PC 與 framebuffer 雜湊遭拒，保留 CPU 暫存器收據。
  一般完整 Go 回歸（含外部 68000 語料）及 vet 通過；原片測試維持失敗。
  私人證據保存 `timerb-gate/`；本輪專用容器退出移除，不更改 main、不推送。

- ENTER 後續：規格 154 修正 IKBD 冷啟動及既有控制器 reset 完成的滑鼠預設，
  不繞過輸入檢查。原先規格 138 清零設定值不符合硬體預設，保留舊收據並訂正契約。
  同片自 reset 五千萬步後正常按下／放開 ENTER，再走 101 萬步，畫面呈現開門動畫。
  Timer A IACK 4727 次，framebuffer SHA-256
  `7a092861ba0f516c800bd25cb219549c4b4cb4032c8f45fa5e7af9364e13bb26`。
  這證明 ENTER 已被接受，不等於完成地城對拍。
- 回歸測試包含明確 cold reset 後不送 Initmous 的滑鼠封包及控制器重設恢復預設。
  舊合成測試的未重設物件仍是零值，沒有為它改變正式建構契約。
  完整 Go 測試（含外部 68000 語料）、vet 與同片 ENTER 乾淨重跑通過；
  私人收據保存於 `workplace/verification/st12-20260908/ikbd/`。
  本輪專用容器全部退出移除，未變更 main，未推送遠端。
...
- 新增私人磁片檢查 `TestPrivateRawFloppy`；Automation 097 實跑以
  `raw floppy length 839680, BPB requires 819200` 拒絕掛載。
  原始磁片未變更、不入版控。研究現況見
  `docs/dm-raw-disk-verification-20260908.md`。本輪一次性容器均已退出移除。
...
- 解開第 74 輪的 `flopvbl()`（規格 139）。第一層是 `flopvbl()` 收尾寫死了 `$23`——
  Hatari 的 `psg_write,fdc` trace 裡 `fdc change drive/side` 那一行同時印舊值與新值，
  整份 trace 每一輪都成對，而且 `$23`／`$25`／`$27` 三種進場值都出現過；EmuTOS 的
  `$FC36B8 set_psg_porta` 回傳舊值的低三位，呼叫端就是拿它還原的。前 73 輪碰巧都是
  `$23`，第 74 輪媒體確認留下 `$25` 才露餡。
- 第二層比較深：`set_psg_porta` 的前三步是 `flopvbl()` 與媒體確認**共用的**，而先前
  讓媒體確認在 select 那一步就認領（判準是「R14 目前是 `$25`」）。drive 0 那一輪兩邊
  都寫 `$25`，前面根本分不出來，分派因此移到下一個 DMA control：`$0084` 是媒體確認的
  sector selector、`$0080` 是 `flopvbl()` 的 status 讀取。drive 寫入的 clock 先存著，
  分派時才寫進收據。
- 開機從 8,058,248 條推進到 **10,544,770 條／209,189,796 clocks**：`flopvbl()` 多走三輪
  （checks 73 → 76）、媒體確認多走兩輪（ring 6 → 8）。新的第一失敗點是同一支
  `set_psg_porta` 寫一個還沒建模的 port A 值，Hatari 的 VBL 1418 顯示那是 `$27`。
- 探針這次踩到兩個坑，都記在做法上：fault 之後才放行沒有用（CPU 已經帶著壞掉的
  prefetch 往前，那條指令不會重跑），要一開始就放行；放行的分支必須擺在既有規則的
  **fallback 位置**，擺在最前面會把狀態機整個接管，開機反而更早停。

- 開機路徑通了：**EmuTOS 1.3 走到 GEM 桌面**（規格 140）。卡在第 77 輪 `flopvbl()` 的
  根因不是少建模一個值，而是模型多猜了一件事——用「已經跑完幾輪」推這一輪要檢查哪個
  drive。Hatari 的 trace 拆穿它：VBL 242–610 之間磁碟在忙，連續 **47 個**（奇數）輪詢
  時槽被跳過，而 VBL 618 恢復時 drive 的奇偶完全沒翻——輪替是 ROM 內部隨 VBL 自由前進
  的計數，跳過的時槽照樣算。機器端看不到那個計數器，硬猜只會在下一次跳過時再錯一次，
  所以改成從 data 那一步把 ROM 寫的值記下來。
- 同一片補上一次性的 deselect：VBL 1130 的完整形狀是「選 drive 1 → 讀 status →
  還原 `$25` → 再一次 `set_psg_porta` 寫 `$27`」，最後那次前後沒有任何 FDC 存取。
  之後每一輪都以 `$27` 進場、還原成 `$27`——那部分規格 139 早就支援，所以接上去就通了。
- 結果比預期遠：不是推進到下一個 gate，而是**沒有下一個 gate**。1.2 億條指令
  （240 億 clocks、343,855 次中斷、18,741 輪 `flopvbl()`）之內沒有再撞到未建模的存取，
  畫面在 1,250 萬條之前就畫完並保持不變——選單列 `Desk File View Options`、
  `DISK A`／`DISK B`、`TRASH` 與滑鼠游標。收據釘在 `$F8000` 起 32,000 bytes 的
  SHA-256 `1de1eb45…`、解析度 0 與 EmuTOS 預設的十六色色盤。
- 方法上值得留的一條：**gate 停在某個值，不代表缺的就是那個值**。這次先在 fallback
...
- 接上 IKBD 的相對滑鼠上行封包（規格 142），從 `main` 的 `59a5bff` 開新分支做。
  封包格式取自兩份獨立的 Atari IKBD 協定文件，兩份的位元佈局逐字相同：
  `%111110xy`，x 是左鍵、y 是右鍵——依位元位置，`$FA` 是左鍵、`$F9` 是右鍵。
  搜尋引擎的摘要把這兩個講反，規格裡留了痕跡，要推翻得拿出比位元佈局更直接的
  一手證據。
- 驗收沒有靠文件自我證明，**oracle 是 EmuTOS 自己的 VDI**：開機到桌面之後注入
  已知位移，量畫面上游標移動的像素數。桌面剛畫完時箭頭 10×16、左上角 (159,99)，
  所以「與基準相異像素的外接矩形」必須是 `(10+|dx|)×(16+|dy|)`——`(+30,0)` 給
  40×16、`(0,+40)` 給 10×56、`(+20,+10)` 給 30×26，而且每次移回原點畫面與基準
  逐像素相同。方向另外用畫面邊界釘死：往負方向推 400 像素貼到 `x=0`、往正方向推
  800 像素貼到 `(319,199)`。整條驗收不碰任何 ROM 版本專屬的變數位址。
- 過程中挖出一個模型自身的缺陷：CPU 在 `STOP` 時原本直接跳到下一個 VBL
  （一次 157,426 clocks），於是一個封包的三個位元組會在同一次裝置推進裡全部送出，
  主機一個都來不及讀，第二個位元組就撞上「RDRF 還沒清掉」。真硬體上 ACIA 中斷會把
  `STOP` 叫醒，所以 idle 跳躍現在會停在上行位元組的投遞時刻。**這個缺陷在只有下行
  命令的時候看不出來**——ROM 那時是主動輪詢的，不需要被叫醒。方向反過來的那一刻
  才暴露出來。

- 規格 143 把滑鼠按鍵的位元位置從文件推論升級成行為驗證。做法是讓 EmuTOS 的桌面
  自己回答：游標移到 `DISK A` 上，短按左鍵→圖示反白並留著（1,140 個像素，
  範圍 (0,17)–(71,50)）；長按又沒移動→放開時 GEM 取消，畫面回到按下之前
  （0 個像素變動，順帶證明放開那個封包確實送到也被處理了）；同一個位置按右鍵
  →**按下與放開都是 0 個像素變動**，桌面完全不理。
- 這條之所以值得單獨做，是因為規格 142 的按鍵位元只有文件當根據，而搜尋摘要與
  原文的位元佈局剛好相反。**兩份原文一致不等於沒讀錯**——讓 ROM 自己反應一次，
  才把「bit 1 是左鍵」這件事釘死。

- 一口氣做完鍵盤、雙擊與對外契約三片，過程中最有價值的是**鍵盤把一個藏了很久的
  假設挖出來**：按鍵會讓 EmuTOS 彈一聲，那串 PSG 寫入把 R7 從 `$C0` 讀改寫成
  `$FE`，而模型有 15 處把「R7 等於 `$C0`」當成 port A 可用的門檻。正確的判準只有
  高兩位（port 方向），混音位元本來就會變。
- 放寬之後又冒出第二層：6 條「選 R14」的規則**從來沒有真的把 `psgRegisterSelect`
  設成 14**。先前那些規則同時要求「目前選的就是 14」，兩件事剛好互相掩護；
  一旦按鍵聲選過 R12，`set_psg_porta` 的讀回就拿到 R12 而不是 port A。
  這是我這輪自己放寬前提時才暴露出來的——**掩護關係一解除，缺漏就現形**。
- 追這個 bug 用的是 PSG 存取的順序追蹤：把 select／data／read 依序印出來，
  一眼看到 `W sel=14` 之後緊接著 `R sel=12`。比逐條猜快得多。
- 規格 145 的收據刻意配一個負對照：同一個對話框，按 `1`（`$02`）0 個像素變動、
  按 Return（`$1C`）29,256 個像素變動。**只有正對照證明不了掃描碼有被解讀**——
  「送什麼都會關」也會通過正對照。
- 規格 146 的結論是節制的：雙擊在 IKBD 這一層根本不存在，四個封包而已。
  能驗的只有「GEM 認得出兩者不同」，開得起磁碟機視窗要等 sector 讀取路徑。
- 規格 147 把機器接上 `talos-jsonl/1`。ROM 由 `TALOS_TOS_ROM` 決定而不是請求帶
  路徑——契約不該把讀檔權限交給對面。`framebuffer` 只回 32,000 個位元組的雜湊與
  幾何，不回像素。
