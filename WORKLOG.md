...
- 接著完成分派迴圈（規格 055）。從單一常式提升到 fetch-execute 循環之後，可以執行任意
...
- 完成布林族（規格 058）。`land`／`lor`／`bnot` 三支常式都是位元運算，配上 `fjp`／`tjp`
