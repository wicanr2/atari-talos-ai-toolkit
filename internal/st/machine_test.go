...
func TestMachineEmuTOSClearsMFPResetISRs(t *testing.T) {
	...
	for machine.Instructions < 7510 {
		if _, err := machine.Step(); err != nil {
			t.Fatalf("post-ISRB step %d: %v", machine.Instructions+1, err)
		}
	}
	if _, err := machine.Step(); err == nil || machine.Instructions != 7510 ||
		machine.CPU.State.A[0] != 0xfffffa13 {
		t.Fatalf("next IMRA stop instructions=%d A0=%08x err=%v",
