...
func TestMachineEmuTOSWritesMFPResetIERs(t *testing.T) {
	...
	for machine.Instructions < 7494 {
		if _, err := machine.Step(); err != nil {
			t.Fatalf("post-IERB step %d: %v", machine.Instructions+1, err)
		}
	}
	if _, err := machine.Step(); err == nil || machine.Instructions != 7494 ||
		machine.CPU.State.A[0] != 0xfffffa0b {
		t.Fatalf("next IPRA stop instructions=%d A0=%08x err=%v",
