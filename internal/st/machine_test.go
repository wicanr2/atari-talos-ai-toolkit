...
func TestMachineEmuTOSStopsTimerD(t *testing.T) {
	...
	for steps := 0; steps < 200_000 && !reachedMedia(machine.Memory, 1, floppyMediaTimeoutSelector) && nextGate == nil; steps++ {
		_, nextGate = machine.Step()
	}
	if nextGate != nil || machine.Memory.floppyMediaPhase != floppyMediaTimeoutSelector || mediaReceipt(machine.Memory, 1).Sector != 1 ||
		machine.Memory.dmaAddress != 0x001004 || mediaReceipt(machine.Memory, 1).DMAAddressStage != 3 ||
		mediaReceipt(machine.Memory, 1).DMAResetCount != 2 || machine.Memory.dmaSectorCount != 1 ||
		mediaReceipt(machine.Memory, 1).ReadCommand != 0x80 || mediaReceipt(machine.Memory, 1).ReadCommandClock != 106340810 ||
		...
		t.Fatalf("sector-read setup stage=%d sector=%02x address=%06x address-stage=%d resets=%d DMA-count=%d command=%02x command-clock=%d checks=%d instructions=%d interrupts=%d clocks=%d state=%+v err=%v",
			machine.Memory.floppyMediaPhase, mediaReceipt(machine.Memory, 1).Sector, machine.Memory.dmaAddress,
			mediaReceipt(machine.Memory, 1).DMAAddressStage, mediaReceipt(machine.Memory, 1).DMAResetCount,
			machine.Memory.dmaSectorCount, mediaReceipt(machine.Memory, 1).ReadCommand,
			mediaReceipt(machine.Memory, 1).ReadCommandClock, machine.Memory.flopVBLMediaChecks,
			machine.Instructions, machine.Interrupts, machine.Clocks, machine.CPU.State, nextGate)
	}
	for steps := 0; steps < 1_500_000 && !reachedMedia(machine.Memory, 1, floppyMediaSeekDataSelector) && nextGate == nil; steps++ {
		_, nextGate = machine.Step()
	}
	if nextGate != nil || machine.Memory.floppyMediaPhase != floppyMediaSeekDataSelector ||
		mediaReceipt(machine.Memory, 1).TimeoutSelectorClock != 118354092 ||
		mediaReceipt(machine.Memory, 1).ForceInterrupt != 0xd0 ||
		mediaReceipt(machine.Memory, 1).ForceInterruptClock != 118354530 ||
		...
		t.Fatalf("force-interrupt complete stage=%d selector-clock=%d force=%02x force-clock=%d FDC/status/type/IRQ/GPIP=%02x/%02x/%v/%v/%02x instructions=%d interrupts=%d clocks=%d state=%+v err=%v",
			machine.Memory.floppyMediaPhase, mediaReceipt(machine.Memory, 1).TimeoutSelectorClock,
			mediaReceipt(machine.Memory, 1).ForceInterrupt, mediaReceipt(machine.Memory, 1).ForceInterruptClock,
			machine.Memory.fdcCommand, machine.Memory.fdcStatus, machine.Memory.fdcStatusTypeI,
			machine.Memory.fdcIRQ, machine.Memory.mfpGPIPIn, machine.Instructions,
			machine.Interrupts, machine.Clocks, machine.CPU.State, nextGate)
	}
	for steps := 0; steps < 10_000 && machine.Memory.floppyMediaReceipts.Total < 1 && nextGate == nil; steps++ {
		_, nextGate = machine.Step()
	}
	if nextGate != nil || machine.Memory.floppyMediaPhase != floppyMediaIdle ||
		mediaReceipt(machine.Memory, 1).SeekData != 0 || mediaReceipt(machine.Memory, 1).SeekCommand != 0x13 ||
		mediaReceipt(machine.Memory, 1).SeekStartClock != 118356450 ||
		mediaReceipt(machine.Memory, 1).InactivePolls != 9 || !mediaReceipt(machine.Memory, 1).IRQObserved ||
		mediaReceipt(machine.Memory, 1).StatusReadClock != 118357766 ||
		machine.Memory.fdcCommand != 0x13 || machine.Memory.fdcStatus != 0xe4 ||
		!machine.Memory.fdcStatusTypeI || machine.Memory.fdcIRQ || machine.Memory.mfpGPIPIn != 0xb1 ||
		...
		t.Fatalf("retry seek complete stage=%d data=%02x command=%02x start=%d polls=%d observed=%v status-read=%d FDC/status/type/IRQ/GPIP=%02x/%02x/%v/%v/%02x instructions=%d interrupts=%d clocks=%d state=%+v err=%v",
			machine.Memory.floppyMediaPhase, mediaReceipt(machine.Memory, 1).SeekData,
			mediaReceipt(machine.Memory, 1).SeekCommand, mediaReceipt(machine.Memory, 1).SeekStartClock,
			mediaReceipt(machine.Memory, 1).InactivePolls, mediaReceipt(machine.Memory, 1).IRQObserved,
			mediaReceipt(machine.Memory, 1).StatusReadClock, machine.Memory.fdcCommand,
			machine.Memory.fdcStatus, machine.Memory.fdcStatusTypeI, machine.Memory.fdcIRQ,
	...
	for steps := 0; steps < 1_000 && !reachedMedia(machine.Memory, 2, floppyMediaTimeoutSelector) && nextGate == nil; steps++ {
		_, nextGate = machine.Step()
	}
	if nextGate != nil || machine.Memory.floppyMediaPhase != floppyMediaTimeoutSelector ||
		mediaReceipt(machine.Memory, 2).Sector != 1 || machine.Memory.dmaAddress != 0x001004 ||
		mediaReceipt(machine.Memory, 2).DMAAddressStage != 3 ||
		mediaReceipt(machine.Memory, 2).DMAResetCount != 2 || machine.Memory.dmaSectorCount != 1 ||
		mediaReceipt(machine.Memory, 2).ReadCommand != 0x80 ||
		mediaReceipt(machine.Memory, 2).ReadCommandClock != 118371398 ||
		...
		t.Fatalf("retry setup stage=%d sector=%02x address=%06x address-stage=%d resets=%d count=%d command/clock=%02x/%d FDC/status/type/IRQ/GPIP=%02x/%02x/%v/%v/%02x checks=%d instructions=%d interrupts=%d clocks=%d state=%+v err=%v",
			machine.Memory.floppyMediaPhase, mediaReceipt(machine.Memory, 2).Sector, machine.Memory.dmaAddress,
			mediaReceipt(machine.Memory, 2).DMAAddressStage, mediaReceipt(machine.Memory, 2).DMAResetCount,
			machine.Memory.dmaSectorCount, mediaReceipt(machine.Memory, 2).ReadCommand,
			mediaReceipt(machine.Memory, 2).ReadCommandClock, machine.Memory.fdcCommand, machine.Memory.fdcStatus,
	...
	for steps := 0; steps < 1_500_000 && !reachedMedia(machine.Memory, 2, floppyMediaSeekDataSelector) && nextGate == nil; steps++ {
		_, nextGate = machine.Step()
	}
	if nextGate != nil || machine.Memory.floppyMediaPhase != floppyMediaSeekDataSelector ||
		mediaReceipt(machine.Memory, 2).TimeoutSelectorClock != 130385964 ||
		mediaReceipt(machine.Memory, 2).ForceInterrupt != 0xd0 ||
		mediaReceipt(machine.Memory, 2).ForceInterruptClock != 130386402 ||
		machine.Memory.fdcCommand != 0xd0 || machine.Memory.fdcStatus != 0x80 ||
		machine.Memory.fdcStatusTypeI || machine.Memory.fdcIRQ || machine.Memory.mfpGPIPIn != 0xb1 ||
		mediaReceipt(machine.Memory, 1).TimeoutSelectorClock != 118354092 ||
		mediaReceipt(machine.Memory, 1).ForceInterruptClock != 118354530 ||
		...
		t.Fatalf("retry timeout stage=%d selector-clock=%d force/clock=%02x/%d FDC/status/type/IRQ/GPIP=%02x/%02x/%v/%v/%02x instructions=%d interrupts=%d clocks=%d state=%+v err=%v",
			machine.Memory.floppyMediaPhase, mediaReceipt(machine.Memory, 2).TimeoutSelectorClock,
			mediaReceipt(machine.Memory, 2).ForceInterrupt, mediaReceipt(machine.Memory, 2).ForceInterruptClock,
			machine.Memory.fdcCommand, machine.Memory.fdcStatus, machine.Memory.fdcStatusTypeI,
			machine.Memory.fdcIRQ, machine.Memory.mfpGPIPIn, machine.Instructions, machine.Interrupts,
			machine.Clocks, machine.CPU.State, nextGate)
	}
	for steps := 0; steps < 10_000 && machine.Memory.floppyMediaReceipts.Total < 2 && nextGate == nil; steps++ {
		_, nextGate = machine.Step()
	}
	if nextGate != nil || machine.Memory.floppyMediaPhase != floppyMediaIdle || mediaReceipt(machine.Memory, 2).SeekData != 0 ||
		mediaReceipt(machine.Memory, 2).SeekCommand != 0x13 ||
		mediaReceipt(machine.Memory, 2).SeekStartClock != 130388322 ||
		mediaReceipt(machine.Memory, 2).InactivePolls != 9 || !mediaReceipt(machine.Memory, 2).IRQObserved ||
		mediaReceipt(machine.Memory, 2).StatusReadClock != 130389638 ||
		machine.Memory.fdcCommand != 0x13 || machine.Memory.fdcStatus != 0xe4 ||
		!machine.Memory.fdcStatusTypeI || machine.Memory.fdcIRQ || machine.Memory.mfpGPIPIn != 0xb1 ||
		mediaReceipt(machine.Memory, 1).SeekStartClock != 118356450 ||
		mediaReceipt(machine.Memory, 1).StatusReadClock != 118357766 ||
		...
		t.Fatalf("second dummy seek stage=%d data=%02x command/start=%02x/%d polls=%d observed=%v status-read=%d FDC/status/type/IRQ/GPIP=%02x/%02x/%v/%v/%02x instructions=%d interrupts=%d clocks=%d state=%+v err=%v",
			machine.Memory.floppyMediaPhase, mediaReceipt(machine.Memory, 2).SeekData,
			mediaReceipt(machine.Memory, 2).SeekCommand, mediaReceipt(machine.Memory, 2).SeekStartClock,
			mediaReceipt(machine.Memory, 2).InactivePolls, mediaReceipt(machine.Memory, 2).IRQObserved,
			mediaReceipt(machine.Memory, 2).StatusReadClock, machine.Memory.fdcCommand,
			machine.Memory.fdcStatus, machine.Memory.fdcStatusTypeI, machine.Memory.fdcIRQ,
			machine.Memory.mfpGPIPIn, machine.Instructions, machine.Interrupts, machine.Clocks,
			machine.CPU.State, nextGate)
	}
	for steps := 0; steps < 100_000 && !reachedMedia(machine.Memory, 3, floppyMediaTimeoutSelector) && nextGate == nil; steps++ {
		_, nextGate = machine.Step()
	}
	if nextGate != nil || machine.Memory.floppyMediaPhase != floppyMediaTimeoutSelector ||
		mediaReceipt(machine.Memory, 3).DrivePort != 0x25 ||
		mediaReceipt(machine.Memory, 3).DriveWriteClock != 130971538 ||
		mediaReceipt(machine.Memory, 3).Sector != 1 || mediaReceipt(machine.Memory, 3).DMAAddressStage != 3 ||
		mediaReceipt(machine.Memory, 3).DMAResetCount != 2 || machine.Memory.dmaSectorCount != 1 ||
		mediaReceipt(machine.Memory, 3).ReadCommand != 0x80 || mediaReceipt(machine.Memory, 3).ReadCommandClock != 130973778 ||
		machine.Memory.fdcCommand != 0x80 || machine.Memory.fdcStatus != 0x81 ||
		machine.Memory.fdcStatusTypeI || machine.Memory.fdcIRQ || machine.Memory.mfpGPIPIn != 0xb1 ||
		machine.Memory.psgRegisters[14] != 0x25 || machine.Memory.flopVBLMediaChecks != 73 ||
		...
		t.Fatalf("third retry stage=%d drive/clock=%02x/%d sector=%d address-stage=%d resets=%d count=%d command/clock=%02x/%d FDC/status/type/IRQ/GPIP=%02x/%02x/%v/%v/%02x R14/checks=%02x/%d instructions=%d interrupts=%d clocks=%d state=%+v err=%v",
			machine.Memory.floppyMediaPhase, mediaReceipt(machine.Memory, 3).DrivePort,
			mediaReceipt(machine.Memory, 3).DriveWriteClock, mediaReceipt(machine.Memory, 3).Sector,
			mediaReceipt(machine.Memory, 3).DMAAddressStage, mediaReceipt(machine.Memory, 3).DMAResetCount,
			machine.Memory.dmaSectorCount, mediaReceipt(machine.Memory, 3).ReadCommand,
			mediaReceipt(machine.Memory, 3).ReadCommandClock, machine.Memory.fdcCommand, machine.Memory.fdcStatus,
			machine.Memory.fdcStatusTypeI, machine.Memory.fdcIRQ, machine.Memory.mfpGPIPIn,
			machine.Memory.psgRegisters[14], machine.Memory.flopVBLMediaChecks, machine.Instructions,
			machine.Interrupts, machine.Clocks, machine.CPU.State, nextGate)
	}
	for steps := 0; steps < 1_500_000 && !reachedMedia(machine.Memory, 3, floppyMediaSeekDataSelector) && nextGate == nil; steps++ {
		_, nextGate = machine.Step()
	}
	if nextGate != nil || machine.Memory.floppyMediaPhase != floppyMediaSeekDataSelector ||
		mediaReceipt(machine.Memory, 3).TimeoutSelectorClock != 142979300 ||
		mediaReceipt(machine.Memory, 3).ForceInterrupt != 0xd0 ||
		mediaReceipt(machine.Memory, 3).ForceInterruptClock != 142979738 ||
		machine.Memory.fdcCommand != 0xd0 || machine.Memory.fdcStatus != 0x80 ||
		machine.Memory.fdcStatusTypeI || machine.Memory.fdcIRQ || machine.Memory.mfpGPIPIn != 0xb1 ||
		...
		t.Fatalf("third timeout stage=%d selector-clock=%d force/clock=%02x/%d FDC/status/type/IRQ/GPIP=%02x/%02x/%v/%v/%02x instructions=%d interrupts=%d clocks=%d state=%+v err=%v",
			machine.Memory.floppyMediaPhase, mediaReceipt(machine.Memory, 3).TimeoutSelectorClock,
			mediaReceipt(machine.Memory, 3).ForceInterrupt, mediaReceipt(machine.Memory, 3).ForceInterruptClock,
			machine.Memory.fdcCommand, machine.Memory.fdcStatus, machine.Memory.fdcStatusTypeI,
			machine.Memory.fdcIRQ, machine.Memory.mfpGPIPIn, machine.Instructions, machine.Interrupts,
			machine.Clocks, machine.CPU.State, nextGate)
	}
	for steps := 0; steps < 10_000 && machine.Memory.floppyMediaReceipts.Total < 3 && nextGate == nil; steps++ {
		_, nextGate = machine.Step()
	}
	if nextGate != nil || machine.Memory.floppyMediaPhase != floppyMediaIdle || mediaReceipt(machine.Memory, 3).SeekData != 0 ||
		mediaReceipt(machine.Memory, 3).SeekCommand != 0x13 ||
		mediaReceipt(machine.Memory, 3).SeekStartClock != 142981658 ||
		mediaReceipt(machine.Memory, 3).InactivePolls != 9 || !mediaReceipt(machine.Memory, 3).IRQObserved ||
		mediaReceipt(machine.Memory, 3).StatusReadClock != 142982974 ||
		machine.Memory.fdcCommand != 0x13 || machine.Memory.fdcStatus != 0xe4 ||
		!machine.Memory.fdcStatusTypeI || machine.Memory.fdcIRQ || machine.Memory.mfpGPIPIn != 0xb1 ||
		...
		t.Fatalf("third dummy stage=%d data=%02x command/start=%02x/%d polls=%d observed=%v status-read=%d FDC/status/type/IRQ/GPIP=%02x/%02x/%v/%v/%02x instructions=%d interrupts=%d clocks=%d state=%+v err=%v",
			machine.Memory.floppyMediaPhase, mediaReceipt(machine.Memory, 3).SeekData,
			mediaReceipt(machine.Memory, 3).SeekCommand, mediaReceipt(machine.Memory, 3).SeekStartClock,
			mediaReceipt(machine.Memory, 3).InactivePolls, mediaReceipt(machine.Memory, 3).IRQObserved,
			mediaReceipt(machine.Memory, 3).StatusReadClock, machine.Memory.fdcCommand,
			machine.Memory.fdcStatus, machine.Memory.fdcStatusTypeI, machine.Memory.fdcIRQ,
			machine.Memory.mfpGPIPIn, machine.Instructions, machine.Interrupts, machine.Clocks,
			machine.CPU.State, nextGate)
	}
