...
func TestMachineEmuTOSWritesMFPResetDDR(t *testing.T) {
	...
	for machine.Instructions < 7486 {
		if _, err := machine.Step(); err != nil {
			t.Fatalf("post-DDR step %d: %v", machine.Instructions+1, err)
		}
	}
	if _, err := machine.Step(); err == nil || machine.Instructions != 7486 ||
		machine.CPU.State.A[0] != 0xfffffa07 {
		t.Fatalf("next IERA stop instructions=%d A0=%08x err=%v",
