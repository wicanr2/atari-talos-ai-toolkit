...
func TestMachineEmuTOSClearsMFPResetIPRs(t *testing.T) {
	...
	for machine.Instructions < 7502 {
		if _, err := machine.Step(); err != nil {
			t.Fatalf("post-IPRB step %d: %v", machine.Instructions+1, err)
		}
	}
	if _, err := machine.Step(); err == nil || machine.Instructions != 7502 ||
		machine.CPU.State.A[0] != 0xfffffa0f {
		t.Fatalf("next ISRA stop instructions=%d A0=%08x err=%v",
